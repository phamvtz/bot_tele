...
export function setupOrderHandlers(bot) {
    ...
    bot.action('menu_orders', async (ctx) => {
        try {
            ...
            orders.forEach((o, i) => {
                ...
                text += `Sản phẩm: ${o.items.map((it) => it.productNameSnapshot).join(', ')}\n\n`;
            });
            // Nếu đơn có trạng thái DELIVERED/COMPLETED, cho phép xem chi tiết lấy Key/Code
            const keyboard = orders
                .filter((o) => o.status === 'COMPLETED' || o.status === 'DELIVERED')
                .map((o) => [{ text: `🔑 Xem kho/Code đơn ${o.orderCode}`, callback_data: `view_delivery_${o.id}` }]);
    ...
    bot.action(/^view_delivery_(.+)$/, async (ctx) => {
        try {
            ...
            if (deliveredItems.length === 0)
                return ctx.answerCbQuery('Đơn này chưa có dữ liệu giao hàng hoặc là dịch vụ.');
