...
checkoutScene.on('text', async (ctx) => {
    if (!ctx.session.waitingForCoupon)
        return;
    ctx.session.waitingForCoupon = false;
    ...
    try {
        ...
        await ctx.reply(`✅ Mã <code>${code}</code> hợp lệ! Giảm <b>${result.discountAmount.toLocaleString('vi-VN')}đ</b>\nCòn lại: <b>${result.finalAmount.toLocaleString('vi-VN')}đ</b>`, { parse_mode: 'HTML' });
        return ctx.scene.reenter();
    }
    catch (e) {
