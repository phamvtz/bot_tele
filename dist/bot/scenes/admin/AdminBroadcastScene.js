...
export const adminBroadcastScene = new Scenes.BaseScene(SCENES.ADMIN_BROADCAST);
adminBroadcastScene.enter(async (ctx) => {
    ctx.session._broadcastStep = 'compose';
    ctx.session._broadcastMsg = undefined;
...
adminBroadcastScene.on('text', async (ctx) => {
    ...
    if (ctx.message.text.startsWith('/'))
        return;
    const step = ctx.session._broadcastStep;
    if (step === 'compose') {
        const message = ctx.message.text;
        ctx.session._broadcastMsg = message;
        ctx.session._broadcastStep = 'confirm';
        // Đếm users sẽ nhận
        const totalUsers = await prisma.user.count({ where: { status: 'ACTIVE' } });
        await ctx.reply(`👀 <b>XEM TRƯỚC:</b>\n${'━'.repeat(24)}\n${message}\n${'━'.repeat(24)}\n\n` +
...
adminBroadcastScene.action('admin:broadcast:yes', async (ctx) => {
    ...
    ctx.session._broadcastStep = undefined;
    ctx.session._broadcastMsg = undefined;
