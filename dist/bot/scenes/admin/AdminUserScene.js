...
export const adminUserScene = new Scenes.BaseScene(SCENES.ADMIN_USER);
// ── Enter: Tìm kiếm user ─────────────────────────────────────────────────────
...
adminUserScene.action(/^admin:balance:add:(.+)$/, async (ctx) => {
    ...
    await ctx.reply(`💰 Nhập số tiền cần <b>CỘNG</b> (VND):\n<i>Ví dụ: 100000</i>`, { parse_mode: 'HTML', reply_markup: Keyboards.backOnly('ADMIN_USER') });
});
// ── Action: Trừ tiền ────────────────────────────────────────────────────────
...
// ── Helper ────────────────────────────────────────────────────────────────────
async function handleBalanceInput(ctx, input) {
    const userId = ctx.session.adminTargetUserId;
    const mode = ctx.session._adjustMode;
    ...
    const wallet = await AdminService.adjustUserBalance(adminId, userId, amount, isAddition, `Admin ${isAddition ? 'cộng' : 'trừ'} ${amount.toLocaleString('vi-VN')}đ`);
    ctx.session._adjustMode = undefined;
    ctx.session.adminTargetUserId = undefined;
    await ctx.reply(`✅ ${isAddition ? 'Cộng' : 'Trừ'} <b>${amount.toLocaleString('vi-VN')}đ</b> thành công!\nSố dư mới: <b>${wallet.balance.toLocaleString('vi-VN')}đ</b>`, { parse_mode: 'HTML' });
}
