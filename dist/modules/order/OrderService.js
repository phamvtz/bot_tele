...
export class OrderService {
    ...
    static async createPendingOrder(userId, productId, quantity, paymentMethod = 'WALLET', couponCode) {
        ...
        if (couponCode) {
            ...
            if (!coupon)
                throw new Error('Mã coupon không tồn tại');
            if (!coupon.isActive)
                throw new Error('Mã coupon đã bị vô hiệu hoá');
            ...
            if (coupon.minOrderAmount && subtotalAmount < coupon.minOrderAmount)
                throw new Error(`Đơn tối thiểu ${coupon.minOrderAmount.toLocaleString('vi-VN')}đ để dùng mã này`);
    ...
    // ─── VIP Auto-Upgrade ──────────────────────────────────────────────────────
    static async checkAndUpgradeVip(userId) {
        const user = await prisma.user.findUnique({
            where: { id: userId },
            include: { vipLevel: true },
        });
        ...
        if (newLevel.spendingThreshold <= currentPriority)
            return;
        await prisma.user.update({
            where: { id: userId },
    ...
    static async validateCoupon(code, amount, userId) {
        const coupon = await prisma.coupon.findUnique({
        ...
        if (coupon.minOrderAmount && amount < coupon.minOrderAmount)
            throw new Error(`❌ Đơn tối thiểu ${coupon.minOrderAmount.toLocaleString('vi-VN')}đ`);
        ...
        if (coupon.perUserUsageLimit) {
            ...
            if (userUsageCount >= coupon.perUserUsageLimit)
                throw new Error('❌ Bạn đã dùng hết lượt cho mã này');
        }
