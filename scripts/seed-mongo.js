...
async function ensureIndexes(db) {
  await Promise.all([
    ...
    db.collection("giftCodes").createIndex({ code: 1 }, { unique: true }),
    db.collection("giftCodeRedemptions").createIndex({ redeemKey: 1 }, { unique: true }),
    db.collection("giftCodeRedemptions").createIndex({ giftCodeId: 1, telegramId: 1 }),
    db.collection("issuedApiKeys").createIndex({ telegramId: 1, createdAt: -1 }),
