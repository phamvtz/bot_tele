...
export default function Settings() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-gray-900 mb-1">Cấu hình Hệ thống</h1>
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1 bg-white rounded-xl border border-gray-200 p-5">
          {activeTab === "shop" && (
            <div>
              <h2 className="text-sm font-semibold text-gray-900 mb-4">Cập nhật Thương hiệu</h2>
              <div className="flex items-start gap-5">
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    <label className="text-xs font-medium text-gray-600 block mb-1">TÊN HIỂN THỊ</label>
                    <input value={f("SHOP_NAME")} onChange={(e) => set("SHOP_NAME", e.target.value)}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30" placeholder="mortal Shop" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-600 block mb-1">MÔ TẢ NGẮN</label>
                    <textarea value={f("SHOP_DESC")} onChange={(e) => set("SHOP_DESC", e.target.value)} rows={2} resize-none
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30 resize-none" placeholder="Mô tả ngắn về shop của bạn..." />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-600 block mb-1">ĐƯỜNG DẪN LOGO (URL)</label>
          ...
          {activeTab === "general" && (
            <div>
              <h2 className="text-sm font-semibold text-gray-900 mb-4">Cài đặt Chung</h2>
          ...
          {activeTab === "security" && (
            <div>
              <h2 className="text-sm font-semibold text-gray-900 mb-4">Bảo mật & Telegram</h2>
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1">Admin Telegram IDs</label>
                  <input value={f("ADMIN_IDS")} onChange={(e) => set("ADMIN_IDS", e.target.value)}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500/30" placeholder="123456789,987654321" />
                  <p className="text-xs text-gray-400 mt-1">Phân cách bằng dấu phẩy. Thay đổi cần restart bot.</p>
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1">Admin Secret (API token)</label>
                  <input type="password" value={f("ADMIN_SECRET")} onChange={(e) => set("ADMIN_SECRET", e.target.value)}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500/30" placeholder="••••••••••••••••" />
                </div>
                <button onClick={() => saveMut.mutate(form)} disabled={saveMut.isPending}
                  className="flex items-center gap-1.5 px-4 py-2 bg-primary-500 text-white rounded-lg text-sm font-medium hover:bg-primary-600 disabled:opacity-50 transition-colors">
                  <Save size={14} />
          ...
          {activeTab === "theme" && (
            <div>
              <h2 className="text-sm font-semibold text-gray-900 mb-4">Giao diện & Chủ đề</h2>
