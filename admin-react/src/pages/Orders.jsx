        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-gray-900">Chi tiết đơn #{detail.id?.slice(-8).toUpperCase()}</h2>
            ...
            <div className="overflow-y-auto flex-1 p-5 space-y-4">
              ...
              {detail.deliveryContent && (
                <div>
                  <p className="text-xs font-semibold text-gray-700 mb-1.5">Nội dung đã giao</p>
              ...
              {detail.deliveryRef && (
                <div>
                  <p className="text-xs text-gray-400">Delivery ref: <code className="bg-gray-100 px-1 rounded text-gray-600">{detail.deliveryRef}</code></p>
                </div>
              )}
                ...
                <div className="flex gap-2 pt-2 border-t border-gray-100">
                  {(STATUS_ACTIONS[detail.status] || []).map(({ label, next, color }) => (
                    <button key={next}
                      ...
                      className={`flex-1 py-2 border rounded-lg text-sm font-medium transition-colors ${color}`}>
                      {label}
                    </button>
                  ))}
                </div>
