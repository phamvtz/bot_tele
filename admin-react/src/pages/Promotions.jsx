      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Tạo coupon mới">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-gray-700 block mb-1">Kiểu giảm</label>
            ...
            <div>
              <label className="text-xs font-medium text-gray-700 block mb-1">Giá trị</label>
              <input type="number" value={form.discountValue} onChange={(e) => setForm((f) => ({...f,discountValue:e.target.value}))}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30" placeholder="10" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-gray-700 block mb-1">Số lần dùng tối đa</label>
              <input type="number" value={form.maxUses} onChange={(e) => setForm((f) => ({...f,maxUses:e.target.value}))}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30" placeholder="Không giới hạn" />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-700 block mb-1">Hết hạn</label>
