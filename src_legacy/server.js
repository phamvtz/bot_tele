...
import { checkAllStock, releaseReservedStock } from "./inventory.js";
    ...
    for (const order of pendingOrders) {
      ...
      if (isOrderExpired(order.createdAt)) {
        await prisma.order.update({
          where: { id: order.id },
          data: { status: "CANCELED", cancelReason: "EXPIRED", canceledAt: new Date() },
        });
  ...
  if (expired.count > 0) {
    const expiredOrders = await prisma.order.findMany({
      where: {
        ...
        canceledAt: { gte: tenMinutesAgo },
      },
      select: { id: true }
    });
    for (const o of expiredOrders) {
      await releaseReservedStock(o.id);
    }
  }
