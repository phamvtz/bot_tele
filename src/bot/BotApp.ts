...
export function createBotApp(token: string): Telegraf<BotContext> {
  ...
  bot.use(async (ctx, next) => {
    if (ctx.callbackQuery) {
      ...
      let answered = false;
      ctx.answerCbQuery = async function (...args: any[]) {
  ...
  bot.action(/^scene:(.+)$/, (ctx) => {
    ...
    if (SCENES[sceneName]) return ctx.scene.enter(SCENES[sceneName]);
  });
  ...
  bot.on('callback_query', async (ctx) => {
    if (!('data' in ctx.callbackQuery)) return ctx.answerCbQuery().catch(() => {});
    ...
    // Map prefix → scene
    const routeMap: Array<[RegExp | string, keyof typeof SCENES]> = [
      [/^shop:/, 'SHOP'],
      [/^back:SHOP$/, 'SHOP'],
  ...
  bot.start(async (ctx) => {
    ...
    if (payload && payload.startsWith('ref_')) {
      await import('../modules/user/UserService.js').then(m =>
        m.UserService.findOrCreateUser(ctx.from.id.toString(), {
  ...
  bot.command('testnotify', adminMiddleware, async (ctx) => {
    ...
    const result = await NotificationService.notifyNewStock({
      ...
      productEmoji: (product as any).thumbnailEmoji ?? '📦',
      addedCount:   10,
      newStockTotal: (product as any).stockCount ?? 10,
          ...
          if (deliveredItems.length > 0) {
            const userMsg =
              ...
              deliveredItems.map(item =>
                `📦 <b>${item.orderItem.productNameSnapshot}</b>\n` +
                `<code>${item.deliveredContent}</code>`
              ).join('\n') +
  ...
  eventBus.onPaymentReceived(async (payload) => {
    if (payload.type === 'DEPOSIT') {
      const wallet = await import('../infrastructure/db.js').then(m =>
        m.default.wallet.findUnique({ where: { userId: payload.userId } })
      );
