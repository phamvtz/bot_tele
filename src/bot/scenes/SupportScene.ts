...
export const supportScene = new Scenes.BaseScene<BotContext>(SCENES.SUPPORT);
...
supportScene.action(/^support:list:(\d+)$/, async (ctx) => {
  ...
  const text = Messages.ticketList(tickets as never, page, totalPages);
