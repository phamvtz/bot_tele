...
export const adminOrderScene = new Scenes.BaseScene<BotContext>(SCENES.ADMIN_ORDERS);
...
adminOrderScene.action(/^admin:order:detail:(.+)$/, async (ctx) => {
  ...
  if (hasDelivered) {
    ...
    deliveredItems.forEach((d: any, i: number) => {
...
adminOrderScene.action(/^admin:order:keys:(.+)$/, async (ctx) => {
  ...
  const texts = keys.map((k: any, i: number) =>
    `📦 <b>Item ${i + 1}</b> — ${k.orderItem?.productNameSnapshot ?? 'N/A'}\n<pre>${k.deliveredContent}</pre>`
  );
...
adminOrderScene.action(/^admin:order:refund:(.+)$/, async (ctx) => {
  ...
  try {
    ...
    await ctx.editMessageText(
      `✅ <b>Đã hủy đơn & hoàn tiền thành công!</b>\n\nĐơn hàng: <code>${orderId}</code>`,
      {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: [
