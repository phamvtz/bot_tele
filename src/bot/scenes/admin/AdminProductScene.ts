...
adminProductScene.action(/^admin:prod:([^:]+)$/, async (ctx) => {
  ...
  if (productId === 'new') {
    ...
    return ctx.editMessageText(
      `📝 <b>TẠO SẢN PHẨM MỚI</b>\n\nBước 1/3: Nhập tên sản phẩm:`,
      { parse_mode: 'HTML', reply_markup: Keyboards.backOnly('ADMIN_PRODUCT') }
    );
  }
  ...
  const text =
    ...
    `📦 Tồn kho: <b>${(product as any).stockMode === 'UNLIMITED' ? '♾️ Vô hạn' : (product as any).stockCount}</b>\n` +
    `📂 Danh mục: <b>${(product as any).category?.name ?? '— Chưa phân loại'}</b>\n` +
...
adminProductScene.action(/^admin:prod:emoji:(.+)$/, async (ctx) => {
  ...
  await ctx.reply(
    ...
    `<i>Bot sẽ tự nhận diện và lưu icon phù hợp.</i>`,
    { parse_mode: 'HTML', reply_markup: Keyboards.backOnly('ADMIN_PRODUCT') }
  );
});
...
adminProductScene.action(/^admin:prod:desc:(.+)$/, async (ctx) => {
  ...
  const product = await ProductService.getProductDetail(productId);
  const current = (product as any)?.shortDescription;

  await ctx.reply(
    `📝 <b>MÔ TẢ SẢN PHẨM</b>\n\n` +
    `${current ? `Mô tả hiện tại:\n<i>${current}</i>\n\n` : ''}` +
    `Nhập mô tả ngắn (1-2 dòng):\n` +
    `<i>Hiển thị trong trang chi tiết sản phẩm ở shop.</i>`,
    { parse_mode: 'HTML', reply_markup: Keyboards.backOnly('ADMIN_PRODUCT') }
  );
});
...
adminProductScene.action(/^admin:prod:price:(.+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `💰 <b>SỬA GIÁ SẢN PHẨM</b>\n\nNhập giá mới (VND)\nVí dụ: <code>50000</code>\nHoặc <code>50000 45000</code> cho giá thường và giá VIP:`,
    { parse_mode: 'HTML', reply_markup: Keyboards.backOnly('ADMIN_PRODUCT') }
  );
});
...
adminProductScene.action(/^admin:prod:cat_pick:([^:]+):(.+)$/, async (ctx) => {
  ...
  (ctx.session as any)._prodStep = undefined;
  ctx.session.adminTargetProductId = undefined;

  await ctx.editMessageText(
    `✅ <b>Đã cập nhật danh mục!</b>\n\n📂 Danh mục: <b>${cat?.name ?? categoryId}</b>`,
    { parse_mode: 'HTML' }
  );
  return ctx.scene.reenter();
});
...
adminProductScene.on('text', async (ctx) => {
  ...
  if (productId === 'NEW') {
    ...
    if (!s._prodName) {
      ...
      await ctx.reply(
        `✅ Tên: <b>${text}</b>\n\nBước 2/3: Nhập giá bán (vd: <code>50000</code>):`,
        { parse_mode: 'HTML', reply_markup: Keyboards.backOnly('ADMIN_PRODUCT') }
      );
      return;
    }
  ...
  if (s._prodStep === 'price') {
    ...
    await ctx.reply(
      `✅ Đã cập nhật giá: <b>${basePrice.toLocaleString('vi-VN')}đ</b>${vipPrice ? `  |  VIP: <b>${vipPrice.toLocaleString('vi-VN')}đ</b>` : ''}`,
      { parse_mode: 'HTML' }
    );
    return ctx.scene.reenter();
  }
  ...
  if (s._prodStep === 'new_icon_custom') {
    if (!s._prodName || !s._prodPrice) return ctx.scene.reenter();
...
adminProductScene.action(/^admin:prod:newcat:(.+)$/, async (ctx) => {
  ...
  for (let i = 0; i < EMOJI_PRESETS.length; i += 4) {
    presetRows.push(
      EMOJI_PRESETS.slice(i, i + 4).map((e, j) =>
        btn(e, `admin:prod:newicon:${i + j}`)
      )
    );
  }
  presetRows.push([
  ...
  await ctx.editMessageText(
    `✅ Tên: <b>${s._prodName}</b>\n💰 Giá: <b>${s._prodPrice.toLocaleString('vi-VN')}đ</b>\n\nBước 4/4: Chọn icon sản phẩm:`,
    { parse_mode: 'HTML', reply_markup: { inline_keyboard: presetRows } }
  );
});
...
adminProductScene.action(/^admin:prod:newicon:(.+)$/, async (ctx) => {
  ...
  if (choice === 'custom') {
    ...
    await ctx.editMessageText(
      `🎭 <b>GỬI EMOJI CỦA BẠN</b>\n\n` +
      `Gửi emoji thường hoặc <b>Emoji Premium</b> động của Telegram.\n` +
      `Bot sẽ tự nhận diện loại emoji.`,
      { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[btn('⬅️ Huỷ', 'back:ADMIN_PRODUCT')]] } }
    );
    return;
  }
...
async function _createProduct(ctx: any, name: string, price: number, categoryId: string | null, emoji = '📦') {
  const slug = name
    .toLowerCase()
  ...
  const replyMethod = ctx.callbackQuery
    ? (t: string, opts: any) => ctx.editMessageText(t, opts)
    : (t: string, opts: any) => ctx.reply(t, opts);

  await replyMethod(
    `✅ <b>TẠO SẢN PHẨM THÀNH CÔNG!</b>\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `📦 Tên: <b>${name}</b>\n` +
    `💰 Giá: <b>${price.toLocaleString('vi-VN')}đ</b>\n` +
    `🎨 Icon: <b>${emoji.startsWith('custom:') ? '<tg-emoji emoji-id="' + emoji.slice(7) + '">📦</tg-emoji>' : emoji}</b>\n` +
    `📂 Danh mục: <b>${catName}</b>\n\n` +
    `<i>Vào chi tiết sản phẩm để nhập kho và bật bán!</i>`,
    { parse_mode: 'HTML' }
  );
