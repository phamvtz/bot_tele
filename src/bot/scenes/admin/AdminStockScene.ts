...
export const adminStockScene = new Scenes.BaseScene<BotContext>(SCENES.ADMIN_STOCK);
...
adminStockScene.on('text', async (ctx) => {
  ...
  if (!productId) {
    return ctx.reply(
      '❌ Chưa chọn sản phẩm.\n\nVui lòng bấm /admin → Nhập Kho rồi chọn sản phẩm trước.',
      { reply_markup: { inline_keyboard: [[{ text: '📥 Nhập Kho', callback_data: 'back:ADMIN_MENU' }]] } }
    );
  }
...
async function promptStockInput(ctx: BotContext) {
  const productId = ctx.session.adminTargetProductId!;
  const product = await ProductService.getProductDetail(productId);
