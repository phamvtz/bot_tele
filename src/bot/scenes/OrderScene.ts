...
function isRateLimited(userId: string, limitMs = 800): boolean {
  const now = Date.now();
  const last = cbMap.get(userId) ?? 0;
  if (now - last < limitMs) return true;
  cbMap.set(userId, now);
  // dọn bộ nhớ mỗi 10 phút
  if (cbMap.size > 2000) cbMap.clear();
  return false;
}

export const orderScene = new Scenes.BaseScene<BotContext>(SCENES.ORDERS);
...
orderScene.enter(async (ctx) => {
  ...
  const text = Messages.orderList(orders as never, page, Math.max(totalPages, 1));
  const keyboard = Keyboards.orderList(orders as never, page, Math.max(totalPages, 1));
...
orderScene.action(/^order:detail:(.+)$/, async (ctx) => {
  ...
  const order = await import('../../infrastructure/db.js').then(m =>
    m.default.order.findUnique({
      where: { id: orderId },
      include: { items: true },
    })
  );

  if (!order) return ctx.answerCbQuery('❌ Không tìm thấy đơn.', { show_alert: true });
...
orderScene.action(/^order:keys:(.+)$/, async (ctx) => {
  ...
  const text = Messages.orderKeys(orderId, deliveredItems as never);
