...
const navBuckets   = new Map<string, RateBucket>();
...
setInterval(() => {
  ...
  for (const [key, bucket] of navBuckets) {
    if (bucket.resetAt <= now) navBuckets.delete(key);
  }
  for (const [key, bucket] of heavyBuckets) {
    if (bucket.resetAt <= now) heavyBuckets.delete(key);
  }
}, 5 * 60_000);

// ─── Helper ───────────────────────────────────────────────────────────────────

function isHeavyAction(callbackData?: string): boolean {
