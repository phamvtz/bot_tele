...
// Format thực tế API trả về:
// { transactionID, amount, description, transactionDate, type: 'IN'|'OUT' }
interface MBBankTransaction {
  transactionID:   string;  // ID giao dịch (unique key)
  amount:          string;  // Số tiền (string)
  description:     string;  // Nội dung CK
  transactionDate: string;  // Ngày giao dịch
  type:            string;  // 'IN' = tiền vào, 'OUT' = tiền ra
  // các field cũ (fallback)
  refNo?:          string;
  tranId?:         string;
  creditAmount?:   string;
  debitAmount?:    string;
...
async function fetchMBBankTransactions(): Promise<MBBankTransaction[]> {
...
async function processMBBankTransaction(tx: MBBankTransaction): Promise<void> {
  ...
  try {
    const result = await PaymentService.processBankCallback(
      'MBBANK',
      uniqueKey,
      credit,
      tx.description,
