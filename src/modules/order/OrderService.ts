...
export class OrderService {
  ...
  static async payWithBank(orderId: string, userId: string) {
    const order = await prisma.order.findUnique({
      where: { id: orderId },
      include: { user: true },
    });
  ...
  static async getUserOrders(userId: string, page = 0, limit = 10) {
  ...
  static async getOrderWithDeliveredItems(orderId: string) {
